...
if __name__ == "__main__":
    for i in range(5):
        ...
        system_prompt = f"""
            ...
            1. 用户的输入要自然、真实、生活化、可包含干扰信息，为每个工具生成一些不同数据集时应保证场景多元化；
            2. 用户的句子较长，可以隐含操作意图，不需要直说工具名；
            3. 必须能通过语义理解正确匹配到工具；
            4. 工具的参数取值必须符合定义的类型与枚举，为每个工具生成一些不同的数据集时应尽量覆盖不同的枚举值；
            5. 输出格式严格如下。
            ...
            2. 单轮对话的用户发言要包含工具相关的关键信息与上下文，但仅有用户说话；
            3. assistant输出的工具信息 与 user 的话严格对应；
            4. 所有字符串用双引号，保证是合法 JSON。
            5. 覆盖所有工具，生成不少于每个工具5条样例。
            
            【目标】
            生成完整的单轮对话测试集，每个工具至少 5 条不同情境的复杂样例，输出为 JSON 数组。
            请将所有样例放入一个 JSON 数组中，不要分成多个数组，不要额外输出任何文字。

            """

        generate_dataset(system_prompt, "complex_single_test_dataset_2.json")
