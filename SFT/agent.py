...
class CustomAgent(BaseLLM):
    ...
    def run(self, input_messages) -> str:
        ...
        # switch to level model adapters
        naive_model = self.model
        self.model = PeftModel.from_pretrained(naive_model,"models/lora_adapters/level")
        ...
        if levels:
            ...
            if level in self.tool_system_prompt:
                ...
                # Switch to tool adapter 
                self.model = PeftModel.from_pretrained(naive_model,f"models/lora_adapters/{level}")
